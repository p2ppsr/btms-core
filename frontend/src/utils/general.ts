...
export const checkAssetBalancesList = (
  ...
  try {
    ...
    const remainderIndex: number = getAssetListIndexFromAsset(remainderList, availableAsset)
    console.log(
    ...
    if (offerList[offerIndex].balance + remainderList[remainderIndex].balance !== availableAsset.balance) {
      console.error(
        `List ${num} ${availableAsset.name}: offered:${offerList[offerIndex].balance} remainer: ${remainderList[remainderIndex].balance} available=${availableAsset.balance}`
      )
    }
    ...
    console.log('offered=', offerList[(availableAsset as any).assetId]?.balance)
    console.log('remainer=', remainderList[(availableAsset as any).assetId]?.balance)
