...
export interface OverlayProvider {
  ...
  submit(body: any): Promise<{ status: string; topics?: Record<string, number[]> }>
}
...
export function createLocalOverlay() {
  ...
  async function requester(url: string, init?: RequestInit): Promise<Response> {
    ...
    if (method !== 'POST') {
      return new SimpleResponse({ error: 'Only POST supported in local overlay' }, { status: 405 }) as any
    }
    ...
    if (path.endsWith('/lookup')) {
      ...
      if (!p) {
        return new SimpleResponse({ error: `Unknown provider ${provider}` }, { status: 404 }) as any
      }
    ...
    return new SimpleResponse({ error: `Unknown path ${path}` }, { status: 404 }) as any
  }
