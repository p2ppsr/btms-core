 ...
 *
 * Field layout (must match listAssets decoding):
 *   0: assetId          (UTF-8)
 *   1: amount           (UTF-8 number string)
 *   2: "ISSUE"          (UTF-8 literal)
 *   3: metadata JSON    (UTF-8)
 */
export class BTMSToken {
  ...
  /**
   * Create the on-chain PushDrop locking script for a BTMS token.
   *
   * REQUIRED FIELD ORDER:
   *   assetId → amount → "ISSUE" → metadata
   *
   * REQUIRED ENCODING:
   *   Utils.toArray(str, "utf8") → number[]
   */
  async lock(
    ...
    // -----------------------------------------------------
    // 1. Encode fields exactly as BTMS decodes them
    // -----------------------------------------------------
    const fields: number[][] = [
      Utils.toArray(assetId, 'utf8'), // assetId
      Utils.toArray(String(amount), 'utf8'), // amount
      Utils.toArray('ISSUE', 'utf8'), // op
      Utils.toArray(metadata, 'utf8') // metadata JSON
    ]
    ...
    const lockScript = await pushdrop.lock(
      ...
      keyID,
      counterparty,
      forSelf // includeSignature defaults to true
    )
    ...
    // -----------------------------------------------------
    // 3. NO SELF-TESTS — decoding belongs in BTMS, not here
    // -----------------------------------------------------

    // -----------------------------------------------------
    // 4. Return actual locking script
    // -----------------------------------------------------
  ...
  /**
   * Standard unlocking logic — delegate to PushDrop.unlock()
   * Matches the HelloTokens design.
   */
...
export class BTMS {
  ...
  private _incomingInFlight = false
  ...
  private async waitForTransportReady(callId: string): Promise<void> {
    for (let i = 0; i < 20; i++) {
      try {
        const v = await walletClient.getVersion()
  ...
  private decodeBTMSToken(scriptHex: string) {
    try {
      ...
      const fields: string[] = rawFields.map((arr: number[]) => Utils.toUTF8(arr))

      // BTMS v2 requires at least [assetId, amount, op, metadataJson]
      if (fields.length < 4) {
  ...
  setActiveAsset(assetId: string) {
    const basket = `${this.basketPrefix} ${assetId}` as BasketStringUnder300Bytes
    this.basket = basket
  ...
  async getTokens(assetId: string): Promise<BTMSWalletOutput[]> {
    ...
    const listResult = await walletClient.listOutputs(args)
  ...
  async getBalance(assetId: string): Promise<number> {
    ...
    // ---------------------------------------------------------
    const basket = `btmstoken v1 ${assetId}` as BasketStringUnder300Bytes
  ...
  async issue(
    ...
    try {
      //
      // 0) PER-TOKEN BASKET: "btmstoken v1 <assetId>"
      //
      const basket = `${this.basketPrefix} ${ASSET_ID_VERSION} ${assetId}` as BasketStringUnder300Bytes

      btmsDebug(`${callId}: using per-token basket`, { basket })

      //
      // 0B) LOAD ISSUER IDENTITY (required for customInstructions)
      //
      ...
      //
      // 1) CREATE LOCKING SCRIPT FOR THE REAL TOKEN
      //
      ...
      const token = new BTMSToken(walletClient)
      const lockScript = await token.lock(this.protocolID, this.protocolKeyID, 'self', assetId, amount, metadataJson)

      const lockingScriptHex = lockScript.toHex()

      //
      // 2) CREATE 1-SAT DISCOVERY OUTPUT
      //
      ...
      //
      // 3) LABEL FOR DISCOVERY
      //
      const discoveryLabel = `${PROTOCOL} ${ASSET_ID_VERSION} ${ASSET_ID_TERM}=${assetId}` as LabelStringUnder300Bytes

      //
      // 4) OUTPUTS (FIXED: full customInstructions)
      //
  ...
  async switchIdentityToActiveProfile() {
    ...
    try {
      ...
      // ----------------------------------------------------------
      const { publicKey: confirmedKey } = await walletClient.getPublicKey({
        identityKey: true
      })
  ...
  async send(
    ...
    try {
      ...
      try {
        beefResult = await walletClient.listOutputs(beefListArgs)
      } catch (err) {
  ...
  async listAssets(includeMode: ListOutputsArgs['include'] = 'locking scripts'): Promise<Asset[]> {
    ...
    btmsDebug(`${callId}: START (basket-based discovery)`, { includeMode })

    // ---------------------------------------------------------
    // STEP 1 — DISCOVER ASSET IDS USING LABELS (CORRECT)
    // ---------------------------------------------------------
    //
    // The discovery basket "btmstoken" contains 1-sat P2PKH outputs.
    // They NEVER contain BTMS v2 scripts.
    //
    // Asset IDs appear *only* in labels:
    //     "btmstoken v1 assetid=<assetId>"
    //
    // DO NOT decode lockingScript here (that is v2 per-asset basket only).
    //
    // ---------------------------------------------------------
    const discoveryArgs: ListOutputsArgs = {
      basket: DISCOVERY_BASKET as BasketStringUnder300Bytes, // "btmstoken"
      include: 'locking scripts',
      includeTags: true,
      includeLabels: true,
      seekPermission: true,
    ...
    btmsDebug(`${callId}: discoveryArgs SENT`, discoveryArgs)

    let discovery: ListOutputsResult
    try {
      discovery = await walletClient.listOutputs(discoveryArgs)
    } catch (err) {
      btmsDebug(`${callId}: listOutputs FAILED`, { err })
      return []
    }

    btmsDebug(`${callId}: discovery RESULT`, {
      totalOutputs: discovery.totalOutputs,
      outputCount: discovery.outputs.length,
      preview: discovery.outputs.slice(0, 5)
    })

    // --- FIX: correct label-based discovery -------------------
    const assetIds = new Set<string>()

    for (const o of discovery.outputs) {
      const labels: string[] = (o as any).labels || []
      for (const lab of labels) {
        // canonical format:
        //    "btmstoken v1 assetid=<assetId>"
        if (lab.startsWith('btmstoken v1 assetid=')) {
          const id = lab.substring('btmstoken v1 assetid='.length).trim()
          if (id.length > 0) assetIds.add(id)
        }
      }
    }

    // ---------------------------------------------------------
    // STEP 1b — FALLBACK: decode lockingScript for v2 baskets
    // ---------------------------------------------------------
    for (const o of discovery.outputs) {
      const scriptHex = this.toLockingScriptHex((o as any).lockingScript, callId, 'listAssets')
      if (!scriptHex) continue

      const decoded = this.decodeBTMSToken(scriptHex)
      if (!decoded.valid) continue

      const id = decoded.assetId
    ...
    btmsDebug(`${callId}: discovered assetIds from labels`, discoveredList)
    ...
    // ---------------------------------------------------------
    // STEP 3 — REAL BALANCES via getBalance() (unchanged)
    // ---------------------------------------------------------
    for (const id of discoveredList) {
  ...
  async acceptIncomingPayment(assetId: string, payment: IncomingPayment): Promise<boolean> {
    ...
    await walletClient.internalizeAction(insertArgs)
  ...
  async listIncomingPayments(assetId?: string): Promise<IncomingPayment[]> {
    ...
    /* ------------------------------------------------------------------ */
    /* 1) Determine correct custody box                                   */
    /* ------------------------------------------------------------------ */

    const custodyBox = `btms-v2-custody-${myIdentityKey}` as LabelStringUnder300Bytes

    btmsDebug(`${callId}: custodyBox`, { custodyBox })

    /* ------------------------------------------------------------------ */
    /* 2) NEW: proper in-flight lock (replaces lastIncomingRun hack)      */
    /* ------------------------------------------------------------------ */

    if (this._incomingInFlight) {
      btmsDebug(`${callId}: SKIP — previous run still in flight`)
      return this._lastIncomingResult ?? []
    }
    this._incomingInFlight = true

    try {
      /* ------------------------------------------------------------------ */
      /* 3) Fetch incoming encrypted messages                               */
      /* ------------------------------------------------------------------ */

      let incoming
      try {
        incoming = await this.tokenator.listMessages({
          messageBox: custodyBox
        })
      } catch (err) {
        btmsDebug(`${callId}: tokenator.listMessages FAILED`, { err })
        return []
      }
      ...
      for (const msg of incoming) {
        try {
          /* -------------------------------------------------------------- */
          /* 3A) Parse body (may be string or object)                       */
          /* -------------------------------------------------------------- */

          let parsed = typeof msg.body === 'string' ? this.safeDoubleParse(msg.body) : msg.body
          ...
          /* -------------------------------------------------------------- */
          /* 3B) LockingScript → hex (TEMP COMPAT)                          */
          /* -------------------------------------------------------------- */

          let lsHex: string | undefined

          if (typeof t.lockingScript === 'string') {
            lsHex = t.lockingScript
          } else if (typeof t.lockingScriptHex === 'string') {
            lsHex = t.lockingScriptHex
          } else if (t.lockingScript && Array.isArray((t.lockingScript as any).bytes)) {
            try {
              const arr = (t.lockingScript as any).bytes
              lsHex = arr.map((b: any) => Number(b).toString(16).padStart(2, '0')).join('')
              btmsDebug(`${callId}: TEMP-COMPAT lockingScript.bytes decoded`, {
                byteLength: arr.length,
                preview: lsHex!.substring(0, 48)
              })
            } catch (e) {
              btmsDebug(`${callId}: TEMP-COMPAT lockingScript.bytes FAILED`, { e })
            }
          }

          if (!lsHex) {
            lsHex = this.toLockingScriptHex(t.lockingScript, callId, 'listIncomingPayments/lockingScript')!
          }

          if (!lsHex) {
            btmsDebug(`${callId}: missing lockingScript`, { t })
            continue
          }

          /* -------------------------------------------------------------- */
          /* 3C) Beef / beefPayload → AtomicBEEF (TEMP COMPAT)              */
          /* -------------------------------------------------------------- */

          let rawBeef = t.beefPayload ?? t.beef

          if (!rawBeef && t.beef && Array.isArray((t.beef as any).bytes)) {
            try {
              rawBeef = (t.beef as any).bytes
              btmsDebug(`${callId}: TEMP-COMPAT beef.bytes used`, {
                byteLength: rawBeef.length
              })
            } catch (e) {
              btmsDebug(`${callId}: TEMP-COMPAT beef.bytes FAILED`, { e })
            }
          }

          const beef = await this.toAtomicBeef(rawBeef, callId, 'listIncomingPayments/beef')

          if (!beef) {
            btmsDebug(`${callId}: cannot normalise beef`, { rawBeef })
            continue
          }

          /* -------------------------------------------------------------- */
          /* 3D) Decode BTMS token (new-world only, no legacy allowed)      */
          /* -------------------------------------------------------------- */

          const d = this.decodeBTMSToken(lsHex)

          if (!d.valid) {
            btmsDebug(`${callId}: decodeBTMSToken FAILED`, { lsHex })
            continue
          }

          // v2 decode result:
          //   { valid, assetId, amount, metadata }
          const tokenName = d.assetId
          const amount = Number(d.amount)
          const metadataJson = d.metadata ?? '{}'
          const op = 'ISSUE' // v2 always ISSUE

          // Asset filter (if caller requested specific asset)
          if (assetId && tokenName !== assetId) continue

          if (!Number.isFinite(amount)) {
            btmsDebug(`${callId}: skip (non-numeric amount)`, { amount })
            continue
          }

          /* -------------------------------------------------------------- */
          /* 3E) Build IncomingPayment                                      */
          /* -------------------------------------------------------------- */
          ...
          btmsDebug(`${callId}: parse error`, { err })
        }
      }

      btmsDebug(`${callId}: COMPLETE`, { count: results.length })

      this._lastIncomingResult = results
      return results
    } finally {
      /* ------------------------------------------------------------------ */
      /* ALWAYS RELEASE LOCK, EVEN ON ERRORS                                */
      /* ------------------------------------------------------------------ */
      this._incomingInFlight = false
    }
  }
...
if (!globalScope.__BTMS_SINGLETON__ || globalScope.__BTMS_SOURCE_TAG__ !== BTMS_SOURCE_TAG) {
  ...
  globalScope.__BTMS_SINGLETON__ = new BTMS()
