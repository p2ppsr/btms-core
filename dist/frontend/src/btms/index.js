...
exports.btms = exports.OverlayClient = exports.setBTMSAuthFetch = exports.sendBTMSToken = exports.acceptBTMSPayment = exports.BTMS = exports.decodeLinkageSimple = exports.createMarketplaceItem = exports.listMarketplaceItems = exports.createLocalToken = void 0;
...
const OVERLAY_BASE = (typeof window !== "undefined" && window.__BTMS_OVERLAY_BASE__) ||
...
async function fetchScriptForOutpoint(outpoint) {
    ...
    if (!txid || Number.isNaN(vout)) {
        if (BTMS_DEBUG) {
            btmsDebug("fetchScriptForOutpoint: invalid txid/vout parsed from outpoint (stub)", {
                outpoint,
                txid,
                voutStr,
            });
    ...
    if (BTMS_DEBUG) {
        btmsDebug("fetchScriptForOutpoint: stub active; no HTTP/overlay lookup performed", {
            outpoint,
            txid,
            vout,
        });
...
/* mint helper */
/* ------------------------------------------------------------------ */
async function walletMint(lockingScript, basket, satoshis, description = "BTMS mint", extra) {
    ...
    if (!isLikelyHex(trimmedLockingScript)) {
        btmsDebug("MINT:walletMint: lockingScript fails hex check", shortHex(trimmedLockingScript));
    }
    ...
    btmsDebug("MINT:walletMint: wallet.createAction durationMs", Date.now() - startedAt);
    if (!action) {
        btmsDebug("MINT:walletMint: createAction returned null — likely validation failure above");
        return null;
    }
    try {
        const a = action;
        ...
        if (atomicBeef) {
            btmsDebug("MINT:walletMint: action has atomic BEEF, broadcasting via HTTPSOverlay + SHIP…");
            ...
            btmsDebug("MINT:walletMint: createAction result had no atomic BEEF (tx) — skipping broadcast");
        }
    }
    catch (e) {
    ...
    async sendMessage(args) {
        ...
        if (BTMS_DEBUG) {
            btmsDebug("MessageBoxTokenator.sendMessage ->", {
                recipient,
                box,
                bodyPreview: typeof payload === "string"
                    ? payload.slice(0, 160)
...
/* ------------------------------------------------------------------ */
class BTMS {
    constructor(tokensMessageBox = "tokens-box", protocolID = [0, "tokens"], basket = "tokens", tokensTopic = "tokens", satoshis = 1, privateKey, marketplaceMessageBox = "marketplace", marketplaceTopic = "marketplace") {
    ...
    async listAssets() {
        ...
        try {
            ...
            for (const o of outs) {
                ...
                let voutIndex = Number.isFinite(o.vout) && o.vout >= 0
                    ? o.vout
                    : 0;
                ...
                if (!scriptHex && BTMS_DEBUG) {
                    btmsDebug(`${callId}: output #${idx} has no script in wallet output; will consider BEEF/customInstructions`, {
                        instanceId: this.instanceId,
                        outpoint,
                        voutIndex,
                        hasBeefMap,
                ...
                if (!scriptHex &&
                    ...
                    try {
                        ...
                        const asUint8 = rawBeefLike instanceof Uint8Array
                            ? rawBeefLike
                            : Uint8Array.from(rawBeefLike);
                        let txFromBeef;
                        try {
                            txFromBeef = sdk_1.Transaction.fromAtomicBEEF(Array.from(asUint8));
                            btmsDebug(`${callId}: parsed atomicBEEF from wallet.BEEF for output #${idx}`, { txid: txidFromOutpoint, voutIndex });
                        }
                        catch (atomicErr) {
                            btmsDebug(`${callId}: atomicBEEF parse failed from wallet.BEEF, trying full BEEF for output #${idx}`, { message: atomicErr?.message, txid: txidFromOutpoint });
                        ...
                        if (out && out.lockingScript) {
                            ...
                            btmsDebug(`${callId}: reconstructed script from wallet.BEEF for output #${idx}`, {
                            ...
                            btmsDebug(`${callId}: wallet.BEEF had tx but no output at voutIndex for output #${idx}`, {
                                txid: txidFromOutpoint,
                                voutIndex,
                        ...
                        btmsDebug(`${callId}: failed to reconstruct script from wallet.BEEF for output #${idx}`, {
                            txid: txidFromOutpoint,
                            message: e?.message,
                ...
                if (!scriptHex) {
                    ...
                    if (decodedFromCI) {
                        ...
                        btmsDebug(`${callId}: output #${idx} decoded as BTMS token via customInstructions`, {
                            assetId: decodedFromCI.assetId,
                    ...
                    btmsDebug(`${callId}: output #${idx} had NO script ANYWHERE (even after BEEF/customInstructions), counted as BSV`, {
                        addedSatoshis: sat,
                        runningBSV: bsvTotal,
                        raw: o,
                        instanceId: this.instanceId,
                ...
                if (decoded) {
                    ...
                    btmsDebug(`${callId}: output #${idx} decoded as BTMS token via script`, {
                        assetId: decoded.assetId,
                        amount: decoded.amount,
                        voutIndex,
                        instanceId: this.instanceId,
    ...
    async acceptIncomingPayment(assetId, payment) {
        ...
        if (!scriptHex && beefPayload) {
            try {
                ...
                const rawBeefLike = beefPayload.atomicBeef ??
                ...
                if (rawBeefLike) {
                    ...
                    if (!out) {
                        btmsDebug(`${callId}: could not find output at vout for reconstructed tx`, { voutIndex, outputs: tx.outputs.length });
                    }
                    else {
                    ...
                    btmsDebug(`${callId}: beefPayload present but no atomicBeef/beef/tx/context field`);
                }
            }
            catch (e) {
        ...
        if (!scriptHex) {
            btmsDebug(`${callId}: no script available on incoming payment (no lockingScript and script reconstruction from beefPayload failed) — acking & failing`, { payment });
            if (payment?.messageId) {
                await this.tokenator.acknowledgeMessage({
                    messageIds: [payment.messageId],
                });
            }
            throw new Error("Incoming payment is missing a spendable lockingScript and script reconstruction from beefPayload failed");
        }
        ...
        if (parsed.lockingPublicKey) {
            ...
            if (myKeyHex) {
                ...
                if (myKeyHex !== normalizedLock) {
                    btmsDebug(`${callId}: locking key mismatch — TEMPORARILY ALLOWING token anyway`, {
                        mine: myKeyHex,
        ...
        try {
            ...
            const rawAtomic = beefPayload && beefPayload.atomicBeef !== undefined
                ? beefPayload.atomicBeef
                : null;
            btmsDebug(`${callId}: raw beefPayload.atomicBeef snapshot`, {
                ...
                tag: rawAtomic && rawAtomic.constructor
                    ? rawAtomic.constructor.name
                    : null,
                sample: Array.isArray(rawAtomic) || rawAtomic instanceof Uint8Array
            ...
            if (Array.isArray(rawAtomic) || rawAtomic instanceof Uint8Array) {
                try {
                    const asUint8 = rawAtomic instanceof Uint8Array
                        ? rawAtomic
                    ...
                    try {
                        ...
                        btmsDebug(`${callId}: atomicBEEF parse failed for wallet internalize, trying full BEEF`, { message: atomicErr?.message });
                        txFromBeef = sdk_1.Transaction.fromBEEF(Array.from(asUint8));
                    }
                    ...
                    btmsDebug(`${callId}: FAILED to canonicalize beefPayload.atomicBeef for wallet internalize`, { message: e?.message });
                }
            }
            else if (rawAtomic != null) {
                btmsDebug(`${callId}: unexpected atomicBeef shape (not array/Uint8Array)`, { value: rawAtomic });
            }
            if (!atomicBeef.length) {
                btmsDebug(`${callId}: wallet internalize skipped — missing/invalid beefPayload.atomicBeef`, { payment });
                // we still continue to ack at the end
                return;
            }
            // 7c) Call wallet.internalizeAction as a basket insertion
            const walletLabels = labels && labels.length ? labels : [actualAssetId || "btms-token"];
            ...
            if (internalizeResult?.accepted) {
                ...
                btmsDebug(`${callId}: wallet.internalizeAction did NOT report accepted:true`);
            }
        }
        catch (err) {
    ...
    async send(...raw) {
        // ---------- helpers ----------
        const isAtomicBEEFArray = (bp) => (Array.isArray(bp) &&
        ...
        const isNonEmptyBeef = (bp) => {
            ...
            if (isAtomicBEEFArray(bp))
                return bp.length > 0;
            if (Array.isArray(bp.atomicBeef))
                return bp.atomicBeef.length > 0;
        ...
        if (!beefArray.length) {
            throw new Error(`BTMS.send: beefPayload empty. OVERLAY_BASE=${OVERLAY_BASE}, haveOutpoint=${haveOutpoint}, txid=${txidNorm}, vout=${voutNorm}.`);
        }
        // -------------------------------------------------------------------------
    ...
    async findFromTokenOverlay(token) {
        ...
        const httpFetcher = typeof fetch === "function" ? fetch.bind(globalThis) : this.requester;
        try {
            ...
            if (json?.type === "output-list" && Array.isArray(json.outputs)) {
                ...
                const normalized = outputs.flatMap((out) => {
                    try {
                        ...
                        if (!Array.isArray(beef) || !beef.length)
                            return [];
                        ...
                        btmsDebug("findFromTokenOverlay: skipping output due to parse error", {
                ...
                if (!normalized.length) {
                    btmsDebug("findFromTokenOverlay: HTTP output-list but no parsable entries", {
                        overlayUrl,
                        txid: token.txid,
                        vout: token.vout,
                    });
    ...
    async issue(...rawArgs) {
        ...
        if (rawArgs.length && typeof rawArgs[0] !== "object") {
            ...
            const lockingScriptHex = typeof lockingScriptObj.toHex === "function"
            ...
            const action = await walletMint(lockingScriptHex, this.basket, this.satoshis, `Mint ${assetId} (${amount})`, beefPayload);
            return {
                assetId,
                amount,
                metadata: normalizedMetadata,
...
async function acceptBTMSPayment(beefPayload) {
    ...
    const broadcaster = new sdk_1.SHIPBroadcaster(["tokens"], {
        networkPreset,
    });
...
async function sendBTMSToken(rawArgs) {
    ...
    try {
        ...
        const amt = typeof args.amount === "string" ? Number(args.amount) : args.amount;
