 ...
 */
function normalizeBytes(value: ByteLike | null | undefined | unknown): Byte[] | undefined {
...
export class BTMSStorage {
  ...
  async saveOnAdmit(record: BTMSRecord): Promise<void> {
    ...
    const now: ISOTimestampString = new Date().toISOString() as ISOTimestampString
    const createdAt: ISOTimestampString | Date = record.createdAt != null ? record.createdAt : now

    const lockingScript = normalizeBytes(rawLockingScript) as LockingScriptBytes | undefined
  ...
  async findAll(): Promise<BTMSRecord[]> {
    ...
    return docs.map(d => ({
      ...d,
      lockingScript: normalizeBytes(d.lockingScript) as LockingScriptBytes | undefined,
   ...
   */
  async findByOutpoint(txid: TXIDHexString, vout: number): Promise<BTMSRecord | null> {
    ...
    return {
      ...doc,
      lockingScript: normalizeBytes(doc.lockingScript) as LockingScriptBytes | undefined,
